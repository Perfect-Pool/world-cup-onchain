...
async function main() {
  ...
  if (networkData.ImagePartsPhaseOne === "") {
    console.log(`Deploying ImagePartsPhaseOne...`);

    const ImagePartsPhaseOne = await ethers.getContractFactory("ImagePartsPhaseOne", {
      libraries: {
        ImageBetTexts: imageBetTextsAddress,
      },
    });

    const imagePartsPhaseOne = await ImagePartsPhaseOne.deploy();
    await imagePartsPhaseOne.deployed();
    console.log(`ImagePartsPhaseOne deployed at ${imagePartsPhaseOne.address}`);

    networkData.ImagePartsPhaseOne = imagePartsPhaseOne.address;
    fs.writeFileSync(variablesPath, JSON.stringify(data, null, 2));

  } else {
    console.log(`ImagePartsPhaseOne already deployed at ${networkData.ImagePartsPhaseOne}`);
  }
}
